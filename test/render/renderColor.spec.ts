...
describe('renderColor', () => {
  ...
  test('out-of-range pixels are transparent when isTransparent is set', () => {
    const data = new Float32Array(PIXELS);
    data[0] = -5; // below min
    data[1] = 5; // within range
    data[2] = 15; // above max
    const options = {...baseOptions, colorScale: {...baseOptions.colorScale, isTransparent: true}};

    const result = renderColor(data, options);
    expect(result[3]).toBe(0);
    expect(result[7]).toBe(255);
    expect(result[11]).toBe(0);
  });

  test('out-of-range pixels are clamped to the scale extremes when isTransparent is not set', () => {
    const data = new Float32Array(PIXELS);
    data[0] = -5;
    data[1] = 15;
